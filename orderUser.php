...
{  
   ...
   {  $user = $_SESSION["customerId"];
	global $obj;
	if($user!=false  && $from!=false && $to !=false){
    	//echo "user";
	...
	foreach($result as $row) {
		$tbl.="<tr><td><a id='show' style='color:pink;decoration:none;' class='glyphicon glyphicon-plus'href='#tr".$row['customer_id']."' data-toggle='collapse'></a></td><td>". $row['customer_name'] . "</td><td>". $row['total'] . " EGP</td>
    </tr>";
		...
		foreach($entriesres  as $entriesrow  ) {
          ...
          <a href='orders.php?dId=". $entriesrow['order_id'] ."'>Cancel</a>
         </li>";
	...
	{
		global $obj;
	    $result=$obj->dbselect('customers',array('customers.*'),'WHERE customers.customer_id =$_SESSION["customerId"]');
		//echo "<option> </option>";
		//foreach($result as $row)
	    echo "<option value='".$result['customer_id']."'>".$result['customer_name']."</option>";
	}
